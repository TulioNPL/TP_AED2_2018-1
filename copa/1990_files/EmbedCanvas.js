...
BannerDisplay.prototype.customFontsToLoad = [];
...
BannerDisplay.prototype.loadFontsHelper = function(attr) {
    if (attr.fontType && attr.fontType === this.fontTypes.CUSTOM) {
        var fontUrl;
        if (attr.localUrl) {
            fontUrl = 'fonts/' + attr.localUrl;
        } else {
            fontUrl = '//' + attr.fontUrl + '.ttf';
        }
        this.setCustomFontsToBeLoaded(fontUrl, attr.fontFamily, attr.fontWeight, attr.fontStyle);
    } else {
        if (attr.localUrl) {
            fontUrl = 'fonts/' + attr.localUrl;
        } else {
            fontUrl = '//' + attr.fontUrl;
        }
        this.setFontsToBeLoaded(fontUrl, attr.fontType, attr.fontFamily, attr.fontWeight, attr.fontStyle);
    }
};
...
BannerDisplay.prototype.loadFonts = function(element) {
    ...
    if (attr.labelStyle && attr.labelStyle.fontFamily) {
        this.loadFontsHelper(attr.labelStyle);
    }
    if (attr.fontFamily) {
        this.loadFontsHelper(attr);
    }
    return true;
};

BannerDisplay.prototype.setCustomFontsToBeLoaded = function(fontUrl, fontFamily, fontWeight, fontStyle) {
    if (!fontUrl || !fontFamily || !fontWeight || !fontStyle) {
        return false;
    }

    if (!this.customFontsToLoad[fontUrl]) {
        this.customFontsToLoad[fontUrl] = {
...
BannerDisplay.prototype.addCustomFontsToDom = function() {
    if (!this.customFontsToLoad) {
        return false;
    }

    var fontFacesString = '';
    for (var fontUrl in this.customFontsToLoad) {
        var customFontToLoad = this.customFontsToLoad[fontUrl];
        fontFacesString += '@font-face {\n' +
            'font-family: "' + customFontToLoad.fontFamily + '";\n' +
            'font-style:' + customFontToLoad.fontStyle + ';\n' +
            'font-weight:' + customFontToLoad.fontWeight + ';\n' +
            'src: url("' + fontUrl + '");\n' +
        '}\n';
    }
    if (fontFacesString === '') {
    ...
    styleTag.appendChild(document.createTextNode(fontFacesString));
    this.container.appendChild(styleTag);
};

BannerDisplay.prototype.setFontsToBeLoaded = function(fontUrl, fontType, fontFamily, fontWeight, fontStyle) {
    if (fontType === this.fontTypes.GOOGLE || !fontType) {
        ...
        var font = {
            fontFamily: fontFamily,
            type: this.fontTypes.GOOGLE,
            fontWeights: fontWeight + ',',
        };
        if (!this.fontsToLoad[fontFamily]) {
            this.fontsToLoad[fontFamily] = font;
            return true;
        }

        if (this.fontsToLoad[fontFamily].fontWeights.indexOf(fontWeight) !== -1) {
            return false;
        }
        this.fontsToLoad[fontFamily].fontWeights += fontWeight + ',';
    } else if (fontType === this.fontTypes.BANNERSNACK) {
        if (!this.fontsToLoad[fontFamily]) {
            var font = {
                fontFamily: fontFamily,
                type: fontType,
            ...
            this.fontsToLoad[fontFamily + '_' + fontStyle + '_' + fontWeight] = font;
        }
    }
    return true;
};
...
BannerDisplay.prototype.addFontsToDom = function() {
    ...
    var googleFonts = [];
    var bannersnackFonts = [];
    for (var font in this.fontsToLoad) {
        if (this.fontsToLoad.hasOwnProperty(font)) {
            var fontType = this.fontsToLoad[font].type;
            if (fontType === this.fontTypes.BANNERSNACK) {
                bannersnackFonts.push(this.fontsToLoad[font]);
            } else if (fontType === this.fontTypes.GOOGLE) {
                googleFonts.push(this.fontsToLoad[font]);
            }
        }
    }
    ...
    if (googleFonts.length) {
        ...
        googleFonts.forEach(function(font) {
            fonts.push([font.fontFamily.replace(/\s/g, '+') + ':' + font.fontWeights.replace(/\,$/, '')]);
        });
    ...
    if (bannersnackFonts.length) {
        var self = this;
        var fontFacesString = '';
        bannersnackFonts.forEach(function(font) {
            fontFacesString += '@font-face {\n' +
        ...
        var styleTag = document.createElement('style');
        styleTag.appendChild(document.createTextNode(fontFacesString));
        self.container.appendChild(styleTag);
    }
...
ButtonDisplay.prototype.init = function(elementData) {
    ...
    properties.labelStyle.fontFamily = '"' + properties.labelStyle.fontFamily + '"';
...
/**
 * Created by laviniuolaru on 23/06/2017.
 */
...
SvgDisplay.prototype.init = function(elementData) {
    var properties = elementData.properties;
    this.properties = properties;
    this.container = this.createElement("div", "element");
    this.displayContainer = this.createElement("div", "svg");
    var img = this.createElement("img", "svg-img");
    this.displayContainer.appendChild(img);
    this.container.appendChild(this.displayContainer);

    var flip = this.getFlipString(properties.flip);
    this.transform("translateZ(0) rotate(" + properties.rotation + "deg) " + flip, this.displayContainer);

    this.applyFilters(
        this.displayContainer,
        properties.adjustColor,
        properties.blur,
        properties.dropShadow
    );

    var url;
    if (properties.localUrl) {
        url = 'images/' + properties.localUrl;
    } else {
        url = bannerConfig.s3Url + 'files/' + properties.url;
    }
    img.src = url;
    this.img = img;
