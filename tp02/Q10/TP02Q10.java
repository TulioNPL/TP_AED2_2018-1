...
*Data: 21/03/2018
*Objetivo: Fazer uma ordenacao por heapsort parcial com K igual a 10 na classe Ranking
********************************************************************************/
...
*/
	...
	*/
	public void ordenaAlfabetico() throws Exception{
		...
		for (int i = (n - 1); i > 0; i--) {
				...
				if(array[j].getDerrotas() == array[j+1].getDerrotas()){
					if(posCharDif < 0){
	...
	*/
	...
	/*
	*Metodo: heapsort()
	*Funcao: fazer uma ordenacao com base no numero de derrotas
	*/
	public void heapsort() throws Exception{
		...
		//Alterar o vetor ignorando a posicao zero
      		...
      		for(int i = 0; i < n; i++){
         		tmp[i+1] = array[i].clonaTime();
      		}//fim for
      		array = tmp;
 
      		for(int tamHeap = 2; tamHeap <= n; tamHeap++){	//Controi o heap
         		constroi(tamHeap);
      		}//fim for
 
      		int tamHeap = n;
      		while(tamHeap > 1){	//Ordena o heap
         		swap(1, tamHeap--);
         		reconstroi(tamHeap);
      		}//fim while
      		...
      		array = new Time[n];
      		for(int i = 0; i < n; i++){
         		array[i] = tmp[i+1].clonaTime();
      		}//fim for
	}//fim heapsort()

	/*
	*Metodo: constroi()
	*Funcao: Cria o heapsort
	*/
	public void constroi(int tamHeap) throws Exception{
		for(int i = tamHeap; i > 1 && array[i].getDerrotas() > array[i/2].getDerrotas(); i /= 2){
         		swap(i, i/2);
			comparacoes++;
      		}//fim for
	}//fim constroi()
		...
		int i = 1, 
		    filho;
      		...
      		while(i <= (tamHeap/2)){
         		...
         		if (array[2*i].getDerrotas() > array[2*i+1].getDerrotas() || 2*i == tamHeap){
            			filho = 2*i;
         		}//fim if 
			else {
            			filho = 2*i + 1;
			...
			comparacoes++;
         		...
         		if(array[i].getDerrotas() < array[filho].getDerrotas()){
            			swap(i, filho);
            			i = filho;
         		}//fim if
			else{
...
class TP02Q10{
	...
	*/
	public static void escreveDados(long tempo) throws Exception{
		File arq = new File("605286_heapsort.txt");
