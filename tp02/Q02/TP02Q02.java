...
*Data: 04/04/2018
*Objetivo: Refazer a questão 03 do TP01 - Parte 02 usando Pilha flexível
***********************************************************************************************/
...
class Pilha {
    	...
    	public Partida remover() throws Exception {
        	...
        	Partida resp = topo.elemento.retornaClone();
      		Celula tmp = topo;
        	topo = topo.prox;
      		tmp.prox = null;
      		tmp = null;
...
class TP02Q02{
	...
	* @param Pilha pilha, Uma pilha de partidas
	*/
	public static void executor(String comando, Pilha pilha) throws Exception{
		...
		switch(comandos[0].charAt(0)){
			...
			case 'I':
				...
				pilha.inserir(partida[pos-1]);		
				break;
			case 'R':
				Partida r = pilha.remover();
	...
	public static void main(String[] args) throws Exception{
		...
		for(int i = 0; i < qtdCopas; i++){	//ciclo contrutor da pilha
			Partida[] partida = new Partida[100];
                        int qtdPartida = Partida.ler(copas[i],partida);

			for(int j = 0; j < qtdPartida; j++){
				pilha.inserir(partida[j]);
			}//fim for
                }//fim for
