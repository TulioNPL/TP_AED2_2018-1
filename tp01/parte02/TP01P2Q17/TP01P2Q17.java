...
*Data: 21/03/2018
*Objetivo: Fazer uma ordenacao por mergesort na classe Ranking
********************************************************************************/
...
*/
	...
	*/
	public void ordenaAlfabetico() throws Exception{
		...
		for (int i = (n - 1); i > 0; i--) {
				...
				if(array[j].getJogos() == array[j+1].getJogos()){
					if(posCharDif < 0){
	...
	*/
	...
	/*
	*Metodo: mergesort()
	*Funcao: fazer uma ordenacao com base no numero de jogos
	*/
	public void mergesort() throws Exception {
		mergesort(0, n-1);	
	}//fim mergesort()
	public void mergesort(int esq, int dir) throws Exception {
		if (esq < dir) {
            		int meio = esq + (dir - esq) / 2;

            		// Ordena o lado esquedo do array
            		mergesort(esq, meio);
	...
	/*
	*Metodo: merge()
	*Funcao: Combina o array esquerdo com o direito
	*/	
	public void merge(int esq, int meio, int dir) throws Exception{
		Time[] temp = new Time[n];

		// Copia as duas partes no array temp
        	for (int i = esq; i <= dir; i++) {
            		temp[i] = array[i].clonaTime();
        	}//fim for
        	...
        	// Copia os menores valores tanto da esquerda quanto da direita
        	// para o array original
        	while (i <= meio && j <= dir) {
            		if (temp[i].getJogos() <= temp[j].getJogos()) {
                		array[k] = temp[i].clonaTime();
                		i++;
            		}//fim if 
			else {
                		array[k] = temp[j].clonaTime();
                		j++;
            		}//fim else
			comparacoes++;
            		k++;
        	...
        	// Copia o resto do lado esquerdo a array para o array original
        	while (i <= meio) {
            		array[k] = temp[i].clonaTime();
            		k++;
            		i++;
...
class TP01P2Q17{
	...
	*/
	public static void escreveDados(long tempo) throws Exception{
		File arq = new File("605286_mergesort.txt");
