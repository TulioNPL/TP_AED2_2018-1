...
*Data: 15/03/2018
*Objetivo: Fazer uma ordenacao por insercao na classe Ranking
********************************************************************************/
...
*/
	...
	*Funcao: ordenar alfabeticamente o array sem desfazer a ordenacao por numero de vitorias
	*/
	public void ordenaAlfabetico() throws Exception{
		...
		for (int i = (n - 1); i > 0; i--) {
				...
				if(array[j].getVitorias() == array[j+1].getVitorias()){
					if(posCharDif < 0){
	...
	*/
        ...
        */
        public void ordenaSelecao() throws Exception{
                for (int i = 0; i < (n - 1); i++) {
                        ...
                        for (int j = (i + 1); j < n; j++){
                                if (array[menor].getVitorias() > array[j].getVitorias()){
                                        menor = j;
                                }//fim if
                                comparacoes++;
                        }//fim for(j)
	...
	/*
	*Metodo: ordenaInsercao
	*Funcao: Faz uma ordenacao por insercao com base no numero de vitorias
	*/
	public void ordenaInsercao() throws Exception{

		for (int i = 1; i < n; i++) {

			Time tmp = array[i].clonaTime();
			int j = i - 1;

			while ( (j >= 0) && (array[j].getVitorias() > tmp.getVitorias()) ){
				array[j + 1] = array[j].clonaTime();
				j--;
				comparacoes++;
			}//fim while

			array[j + 1] = tmp.clonaTime();
		}//fim for
	}//fim ordenaInsercao()
...
class TP01P2Q11{
	...
	*/
	public static void escreveDados(long tempo) throws Exception{
		File arq = new File("605286_insercao.txt");
