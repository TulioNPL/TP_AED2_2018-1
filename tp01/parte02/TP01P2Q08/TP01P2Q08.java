...
*Data: 15/03/2018
*Objetivo: Fazer uma pesquisa binaria na classe Ranking
********************************************************************************/
...
*/
	...
	*Funcao: ordenar alfabeticamente o array 
	*/
	...
	*/
	public int pegaCharDif(int i, int j){
		int posCharDif = 0;
                int menorTam = getMenorString(i, j);
        ...
        public int pegaCharDif(int i, String s){
                int posCharDif = 0;
                int menorTam = getMenorString(i, s);

                for(int cont = 0; cont < menorTam; cont++){ //pega a primeira posicao em que o char eh diferente
                        if(array[i].getPais().charAt(cont) == s.charAt(cont)){
                                posCharDif++;
                        }//fim if
                        else {
                                cont = menorTam;
	...
	/*
	*Metodo: getMenorString()
	*Funcao: pegar to tamanho do pais com menor nome
	*/
	public int getMenorString(int i, int j){
	...
	public int getMenorString(int i, String s){
                int menorTam = array[i].getPais().length();

                if(s.length() < menorTam){
                        menorTam = s.length();
                }//fim if
	...
	/*
	*Metodo: swap()
	*Funcao: trocar duas instancias de time de lugar no array
	*/
	...
	*/
	...
	public boolean pesquisaBinaria(String time1){
		...
		while (esq <= dir) {
			...
			else if (retornaPrimeira(meio,time1).equals(array[meio].getPais())){
				esq = meio + 1;
				comparacoes++;
			}//fim else if 
			else{
	...
	/*
	*Metodo: retornaPrimeira()
	*Funcao: retorna a String que vem primeiro em ordem alfabetica
	*/
	public String retornaPrimeira(int i, String time1){
		String time2 = array[i].getPais();
		String primeira = "";
		int posCharDif = pegaCharDif(i ,time1);
		
		if(posCharDif < 0){
			if(time1.length() > time2.length()) primeira = time2;
			else primeira = time1;
		}//fim if
		else if(time1.charAt(posCharDif) > time2.charAt(posCharDif)){
			primeira = time2;
		}//fim else if
		else if(time1.charAt(posCharDif) < time2.charAt(posCharDif)){
			primeira = time1;
...
class TP01P2Q08{
	...
	*/
	public static void escreveDados(long tempo) throws Exception{
		File arq = new File("605286_Binaria.txt");
	...
	public static void main(String[] args) throws Exception{
		...
		for(int i = 0; i < qtdChaves; i++){
			existe = ranking.pesquisaBinaria(chaves[i]);
			if(existe){
