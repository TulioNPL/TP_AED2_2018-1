...
*Data: 21/03/2018
*Objetivo: Fazer uma ordenacao por countingsort na classe Ranking
********************************************************************************/
...
*/
	...
	*/
	public void ordenaAlfabetico() throws Exception{
		...
		for (int i = (n - 1); i > 0; i--) {
				...
				if(array[j].getGolsContra() == array[j+1].getGolsContra()){
					if(posCharDif < 0){
	...
	*/
	...
	/*
	*Metodo: countingsort()
	*Funcao: fazer uma ordenacao com base no numero de gols Contra
	*/
	public void countingsort() throws Exception{
		//Array para contar o numero de ocorrencias de cada elemento
        	...
        	for (int i = 0; i < count.length; count[i] = 0, i++);
      		...
      		for (int i = 0; i < n; count[array[i].getGolsContra()]++, i++);
 
      		//Agora, o count[i] contem o numero de elemento menores ou iguais a i
      		for(int i = 1; i < count.length; count[i] += count[i-1], i++);
 
      		//Ordenando
      		for(int i = n-1; i >= 0; ordenado[count[array[i].getGolsContra()]-1] = array[i].clonaTime(), count[array[i].getGolsContra()]--, i--);
 
      		//Copiando para o array original
      		for(int i = 0; i < n; array[i] = ordenado[i].clonaTime(), i++);
	}//fim countingsort()

	/*
	*Metodo: getMaior()
	*Funcao: Pegar o maior elemento do array
	*/
	public int getMaior() throws Exception{
		int maior = array[0].getGolsContra();
 
        	for (int i = 0; i < n; i++) {
         		if(maior < array[i].getGolsContra()){
            			maior = array[i].getGolsContra();
         		}//fim if
			comparacoes++;
        	}//fim for
       		return maior;    
...
class TP01P2Q15{
	...
	*/
	public static void escreveDados(long tempo) throws Exception{
		File arq = new File("605286_countingsort.txt");
