...
*Data: 21/03/2018
*Objetivo: Fazer uma ordenacao por shellsort na classe Ranking
********************************************************************************/
...
*/
	...
	*/
	public void ordenaAlfabetico() throws Exception{
		...
		for (int i = (n - 1); i > 0; i--) {
				...
				if(array[j].getEmpates() == array[j+1].getEmpates()){
					if(posCharDif < 0){
	...
	*/
	...
	/*
	*Metodo: shellsort()
	*Funcao: fazer uma ordenacao com base no numero de empates
	*/
	...
	*Funcao: Efetua insercao nos pseudo arrays do shellsort
	*/
	public void insercaoPorCor(int cor, int h) throws Exception{
		for (int i = (h + cor); i < n; i+=h) {
         		...
         		while ((j >= 0) && (array[j].getEmpates() > tmp.getEmpates())) {
            			array[j + h] = array[j].clonaTime();
            			j-=h;
				comparacoes++;
         		}//fim while

         		array[j + h] = tmp.clonaTime();
      		}//fim for			
	}//fim insercaoPorCor()
...
class TP01P2Q12{
	...
	*/
	public static void escreveDados(long tempo) throws Exception{
		File arq = new File("605286_shellsort.txt");
