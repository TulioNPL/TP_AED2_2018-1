...
*Data: 21/03/2018
*Objetivo: Fazer uma ordenacao por radixsort na classe Ranking
********************************************************************************/
...
class Time extends Object implements Cloneable, Comparator<Time> {
	...
	*/
	...
	public int getDoidao(){
		return this.numPontos*1000+golsPro;
	}//fim getDoidao()
...
*/
	...
	*/
	public void ordenaAlfabetico() throws Exception{
		...
		for (int i = (n - 1); i > 0; i--) {
				...
				if(array[j].getDoidao() == array[j+1].getDoidao()){
					if(posCharDif < 0){
	...
	*/
	public int pegaCharDif(int i, int j){
		...
		if(posCharDif == menorTam) posCharDif = -1; //caso chegue ao fim de uma das palavras sem achar diferenca
	...
	*/
	...
	/*
	*Metodo: radixsort()
	*Funcao: fazer uma ordenacao com base no numero doidao
	*/
	public void radixsort() throws Exception {
        	int m = getMax();	// Encontra o maior valor para achar o numero de digitos
	...
	/*
	* Metodo: countSort()	
	* Funcao: Ordenar o array com base no digito representado pelo @param int exp
	*/
	public void countSort(int exp) throws Exception{
		Time output[] = new Time[n]; // array de saida
        	int i;
        	int count[] = new int[10];
        	Arrays.fill(count,0);
 
        	for (i = 0; i < n; i++){
            		count[ (array[i].getDoidao()/exp)%10 ]++;
			comparacoes++;
 		}//fim for

        	for (i = 1; i < 10; i++){	// Reorganiza as posicoes dos digitos em count
            		count[i] += count[i - 1];
			comparacoes++;
 		}//fim for

        	for (i = n - 1; i >= 0; i--){	// Monta o array de saida
            		output[count[ (array[i].getDoidao()/exp)%10 ] - 1] = array[i].clonaTime();
            		count[ (array[i].getDoidao()/exp)%10 ]--;
			comparacoes++;
        	}//fim for
 
        	for (i = 0; i < n; i++){	// Copia o array de saida para o array original
            		array[i] = output[i].clonaTime();
			comparacoes++;
		}//fim for
	}//fim countSort()	
	...
	/*
	*Metodo: getMax()	
	*Funcao: encontrar o maior valor Doidao
	*/
	public int getMax(){
		int max = array[0].getDoidao();

        	for (int i = 1; i < n; i++){
            		if (array[i].getDoidao() > max){
                		max = array[i].getDoidao();
			}//fim if
			comparacoes++;
		}//fim for
...
class TP01P2Q18{
	...
	*/
	public static void escreveDados(long tempo) throws Exception{
		File arq = new File("605286_radixsort.txt");
